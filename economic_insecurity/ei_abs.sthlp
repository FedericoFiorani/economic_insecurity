...
{phang}
{bf:ei_abs} {hline 2} Generates the individual-level absolute economic insecurity index for each observation, consistent with the absolute component of {help economic_insecurity:economic_insecurity}.

{title:Syntax}

{p 8 17 2}
{cmd:egen} {it:newvar} {cmd:=} {cmd:ei_abs(}{it:exp}{cmd:)} [{it:if}] [{it:in}] {cmd:,}
[{opt id(varname)} {opt time(varname)} {opt loss(real)} {opt gain(real)}
{opt discount(real)} {opt periods(integer)} {opt gaps(string)}]

{pstd}
where {it:exp} is a numeric variable or expression measuring income, earnings, wealth, or another resource observed over time. The {opt time()} variable must be coded as a consecutive numeric sequence within the observation window. For biennial or irregularly spaced panels, users should supply a sequential wave variable (for example, 1, 2, 3, ...) rather than raw calendar years; see {help ei_abs##timenote:Note on time coding}.
...
{pstd}
{cmd:ei_abs} does not estimate aggregate means and does not accept weights or the {cmd:svy} option. For sample-level estimation, weighted means, standard errors, and confidence intervals, use {help economic_insecurity:economic_insecurity}.
...
{pstd}
{cmd:ei_abs} computes the individual-level absolute economic insecurity index at each observation in the requested sample. The calculation mirrors the absolute component of {cmd:economic_insecurity}: year-to-year changes in the resource variable are transformed into one-period insecurity values, losses and gains may receive different weights, and past periods are discounted.
...
{phang2}{cmd:. egen ei_a = ei_abs(ln_wage)}{p_end}
{phang2}{cmd:. egen ei_a5 = ei_abs(ln_wage), loss(0.7) gain(0.2) discount(0.1) periods(5)}{p_end}
{phang2}{cmd:. egen ei_a_sel = ei_abs(ln_wage) if year >= 75, id(idcode) time(year) gaps(break)}{p_end}
